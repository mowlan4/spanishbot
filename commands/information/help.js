  ...
  run: async (client, message, args) => {
    ...
    if (query.length === 0) {
      ...
      let embed = new discord.MessageEmbed()
        ...
        .setDescription(
          `Run \`${client.prefix}help category\` to view the list of available commands in that category`
        )

        .addField(
          "Information",
          `General information\nAliases: \`${[...information].join(", ")}\``,
          true
        )
        .addField(
          "Fun",
        ...
        .addField(
          "Moderation",
          `Moderation tools\nAliases: \`${[...moderation].join(", ")}\``,
          true
        )
        .addField(
          "Utility",
          `Random utilities\nAliases: \`${[...utility].join(", ")}\``,
          true
        )
        .setTimestamp()
        .setFooter({
      ...
      if (information.has(query)) {
        category = "information";
      }
      if (moderation.has(query)) {
        category = "moderation";
      }
      if (utility.has(query)) {
        category = "utility";
      }
      if (fun.has(query)) {
        category = "fun";
      ...
      if (category) {
        ...
        if (page < 1) {
          return await inputError(
            client,
            "help",
            message.author,
            message.channel,
            `${page} aka [page #] must be at least 1`
          );
        }
        ...
        client.commands.forEach((command) => {
          if (
            command.category ===
            category /*&& (!message.guild || !command.guildOnly || isOwner(client, message.author.id) || checkPerms(client, command, message.member, message.guild))*/
          ) {
        ...
        if (page > maxPages) {
          return await inputError(
            ...
            message.author,
            message.channel,
            `${insertZeroWidth(page)} aka [page #] must be at most ${maxPages}`
          );
        }
        ...
        let embed = new discord.MessageEmbed()
          ...
          .setTimestamp()
          .setFooter({
            text: `${message.author.tag} | Page ${page}/${maxPages}`,
            iconURL: message.author.displayAvatarURL(),
          });

        let description = `Run \`${client.prefix}help command\` to view the detailed help message for that command\n\n`;
        for (
          let i = (page - 1) * MAX_COMMANDS_PER_PAGE;
          i < Math.min(page * MAX_COMMANDS_PER_PAGE, commandsInfo.length);
          ++i
        ) {
        ...
        if (
          command /*&& (!message.guild || !command.guildOnly || isOwner(client, message.author.id) || checkPerms(client, command, message.member, message.guild))*/
        ) {
          ...
          return await inputError(
            ...
            `${insertZeroWidth(
              query
            )} aka [category/command] is not a valid category or command`
          );
        }
      }
    }
