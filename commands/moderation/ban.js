    ...
    if (extractedID || user) {
      ...
      if (user) {
        ...
        if (
          ...
          return await message.channel.send(
            `${message.author}, you cannot ban someone who has an equal or higher role than you!`
          );
        }
      }

      try {
        await message.guild.members.ban(extractedID || user, {
          reason: `Banned by ${message.author.tag} (ID: ${message.author.id})`,
        });
        return message.channel.send(
          `Successfully banned ${
            user ? user.tag : client.users.cache.get(extractedID).tag
          } from the server!`
        );
      } catch (err) {
        return message.channel.send(
          `Could not ban ${
            user ? user.tag : extractedID
          } from the server.\n${codeBlock(err.message)}`
        );
      }
    } else {
      return await message.channel.send(
        `Who would you like me to ban? https://images-ext-1.discordapp.net/external/iyNi5ZndLzvuKm0F2IkDlZrWUZzUOaxu73kTHbQRlJ0/%3Fc%3DVjFfZGlzY29yZA/https/media.tenor.com/vkDCjozbDksAAAPo/ban-hammer-cinzou.mp4`
      );
    }
  },
};
